...
const methods = new Set(['snapshot','session.create','session.delete','session.switch','session.pin','session.review','session.read','session.cancelRecovery','terminal.attach','terminal.input','terminal.resize','workspace.save','accounts.import','accounts.refresh','folder.pick','runtime.shutdown']);
